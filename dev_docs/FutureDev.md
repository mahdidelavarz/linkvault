# LinkVault — Future Development Backlog

Items in this file are well-designed but deliberately deferred from the current MVP roadmap. They should be revisited after P4-B (Projects), P4-C (PWA Offline), and the P5 A/B-tier features are stable.

---

## Future: Cross-Module Relations Graph

> Originally planned as P4-B. Replaced by the simpler "Projects" grouping system for MVP.
> This design remains valid for a V2 knowledge-graph feature.
...
#### 1. Infrastructure → API Client (`uses`) — Functional
...
**Start with:** The Infrastructure → API Client connection (functional, highest value). This requires the `item_relations` table, the relation API, and the environment switcher UI in the API Client. Everything else builds on top of the same table.
